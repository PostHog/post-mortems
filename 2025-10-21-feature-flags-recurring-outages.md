# PostHog Feature Flags Service - Multiple Outages (October 2025)
...
- **October 30** (7 hours 9 minutes): CPU-bound latency from node CPU pressure exceeding 90%
...
- **Primary root cause: CPU resource undersizing** - Nodes were not properly sized for the workload, leading to CPU pressure that caused excessive parallelism and connection pool exhaustion
- Same root cause as October 28: CPU undersizing manifested as excessive parallelism and connection pool exhaustion
- **Connection pool management complexity**: The flags service manages connection pools itself, but spinning up new connection pools across pods is computationally expensive, especially when pods become CPU-bound. This exacerbated the connection pool exhaustion issues.
- Symptom-focused fix that didn't address underlying CPU sizing issues
- Unbounded cache population logic with no rate limiting
- Excessive concurrent connections overwhelming database and cache layers (symptom of CPU pressure)
...
- Library's secure defaults (not trusting X-Forwarded-For headers) were inappropriate for our trusted infrastructure
- No alerting configured for 429 errors, requiring customer reports for detection (62-minute detection delay)
- Rate limiting never validated with production architecture including load balancers
...
- **Primary root cause: CPU resource undersizing** - Same root cause as October 21: CPU undersizing caused excessive parallelism and connection pool exhaustion
- **Connection pool management complexity**: New pods attempting to spin up connection pools failed under CPU pressure, as this operation is computationally expensive especially when CPU-bound
- Unrelated deployment triggered feature flags pod rollout
- New pods failing to connect to Postgres within 20s timeout under CPU pressure
- Pods entering crash loops, reducing available capacity
- Redis write storm during deployment causing key evictions
- Shared Redis instance between flags service and main application (critical infrastructure coupling)
...
- CPU pressure on nodes exceeding 90%
...
2. **Connection pool management complexity**: The flags service manages connection pools itself, but spinning up new connection pools across pods is computationally expensive, especially when pods become CPU-bound. This complexity, combined with CPU pressure, exacerbated connection pool exhaustion issues. The solution: reduce concurrency and run smaller fleets with better-resourced pods rather than larger fleets with CPU-bound pods.
...
4. **Critical monitoring gap: CPU alerting was missing**: CPU alerting was completely absent throughout these incidents, preventing early detection of CPU pressure that was the root cause of three outages. This was a fundamental gap in our monitoring strategy that allowed CPU pressure to escalate unnoticed.
...
- **Total downtime:** Over 14 hours across four incidents
...
- **Rate limiting fixes**: Fixed rate-limiting configuration that caused October 24 incident
...
- **CPU right-sizing (critical fix)**: Properly sized CPU resources for nodes running the feature flags service. This was the primary remediation for three of the four incidents (October 21, 28, and 30), addressing the root cause of excessive parallelism, connection pool exhaustion, and CPU-bound latency
- **Infrastructure right-sizing**: Adjusted Kubernetes fleet size, pod connection configuration, and CPU allocation to match actual workload
- **Fleet size and concurrency optimization**: Reduced fleet size and improved pod resource allocation to reduce concurrency and connection pool overhead. Running smaller fleets with better-resourced pods rather than larger fleets with CPU-bound pods, reducing the computational expense of spinning up connection pools.
- **Critical monitoring gap**: CPU alerting was completely missing throughout these incidents, preventing early detection of CPU pressure that was the root cause of three outages. Added comprehensive CPU alerting to catch CPU pressure before it causes service degradation.
...
1. **CPU right-sizing is fundamental** – The biggest takeaway from these incidents: improper CPU resource sizing was the root cause for three of the four outages. CPU pressure led to excessive parallelism, connection pool exhaustion, and slow queries. Right-sizing CPU resources addressed the underlying issues that caused October 21, 28, and 30 incidents. This must be a primary consideration for any service deployment.

2. **Connection pool management architecture matters** – The flags service manages connection pools itself, but spinning up new connection pools across pods is computationally expensive, especially when pods become CPU-bound. This complexity, combined with CPU pressure, exacerbated connection pool exhaustion. Better approach: reduce concurrency and run smaller fleets with better-resourced pods rather than larger fleets with CPU-bound pods.
...
4. **CPU alerting was completely missing** – CPU alerting was absent throughout these incidents, preventing early detection of CPU pressure that was the root cause of three outages. This was a fundamental gap in our monitoring strategy. CPU metrics must be monitored and alertable from day one.
...
7. **Progressive rollouts save lives** – Gradual deployments limit blast radius and enable rapid detection.
